...
export class GoPlay {
    ...
    _toLivestreamMetaPreview(livestream) {
        return {
            ...
            background: livestream.images[0].url,
            website: websiteurl
        }
    }
    ...
    async getStream(video) {
        ...
        if (video.manifestUrls) {
            url = video.manifestUrls.hls || video.manifestUrls.dash;
        }
    ...
    async getVideo(id, type) {
        ...
        switch(type) {
            case 'movie':
                ...
                video = await self.api.getVideoLongForm(program.nextVideo).catch();
                break;
            ...
            case 'tv':
                video = await self.api.getVideoLivestream(self._getGuidFromId(id)).catch();
                break;
        }
    ...
    async getMeta(id, type) {
        const self = this;
        if (type == 'tv') {
           return self.api.getVideoLivestream(self._getGuidFromId(id))
                .then((r) => self._toLivestreamMetaPreview(r));
        }
    ...
    _filterStream(query, livestream) {
        let result = false;
        livestream.title = livestream.title || '';
        query.forEach(element => result = 
            livestream.title.includes(element) ||
            livestream.description.includes(element));
        return result;
    }
    ...
    async search(query, type) {
        ...
        if (type == 'tv') {
            var q = query.split(' ');
            return self.api.getLiveStreams()
                 .then((r) => r.filter((livestream) => self._filterStream(q, livestream))
                    .map(livestream => self._toLivestreamMetaPreview(livestream)));
         }
