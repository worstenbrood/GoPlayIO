...
export class GoPlay {
    ...
    async getAllPrograms(type, skip) {
        ...
        return self.api.getPage(page)
            ...
            .then((r) => r.flat())
            // Throttle this
            .then((r) => parallel(r.map(card => card.uuid), element => self.api.getProgramWeb(element), 4))
            .then((r) => r.filter(element => self._getType(element.type) == type).map(element => self._toMetaPreview(element)));
    }
    ...
    async search(query, type) {
        ...
        return self.api.search(query)
            .then((r) => parallel(r.cards.map(card => card.uuid), element => self.api.getProgramWeb(element), 4))
            .then((r) => r.filter(element => self._getType(element.type) == type).map(element => self._toMetaPreview(element)));
    }
}